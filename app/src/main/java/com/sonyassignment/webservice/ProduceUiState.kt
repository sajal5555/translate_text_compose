 ...
 *
 * [Producer] is any object that has a suspending method that returns [RepoResult]. In the [block] call
...
fun <Producer, T> produceUiState(
    producer: Producer,
    block: suspend Producer.() -> RepoResult<T>
    ...
    val result = produceState(UiState<T>(loading = true), producer, key) {
        ...
        launch {
            ...
            for (clearEvent in errorClearChannel) {
                ...
                value = value.copy(errorMessage = null)
            }
        }
